...
const char *argp_program_version = "capable unstable test code";
...
"    capable -b vmlinux.min   # Specify the custom path of BTF\n"
...
static const struct argp_option opts[] = {
	...
	{ "btf", 'b', "BTF", 0, "Specify the custom path of BTF" },
...
static error_t parse_arg(int key, char *arg, struct argp_state *state)
{
	...
	case 'p':
		...
		break;
	case 'b':
		env.btf_path = strndup(arg, PATH_MAX);
		if (!env.btf_path) {
			argp_usage(state);
		}
		break;
...
static void print_map(struct ksyms *ksyms, struct syms_cache *syms_cache, struct capable_bpf *obj)
{
...
int main(int argc, char **argv)
{
	...
	opts.btf_custom_path = env.btf_path;

	obj = capable_bpf__open_opts(&opts);
	if (!obj) {
